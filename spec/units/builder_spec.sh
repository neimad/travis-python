...
Describe "install_builder()"
    It "fails when the directory is not specified"
        When run install_builder
    ...
    It "installs the builder to the specified directory"
        ...
        stub 'current_builder_version' -o "20200110"

        When call install_builder "$directory"
        The command "update_git_repo https://github.com/pyenv/pyenv /tmp/pyenv" should be called
...
Describe "available_python_versions_with_builder()"
    It "gets the list of available versions from python-build"
        spy 'python-build'

        When call available_python_versions_with_builder
...
Describe "current_builder_version()"
    It "gets the version from python-build output"
        spy 'python-build'

        When call current_builder_version
